...
import { Form, TextButton, Input, Select, TextArea, Button, IconButton, SwitchGroup } from '@matusgallo/mysabds'
...
function PropertySummary({ typNemovitosti, typNabidky, adresa, cenaNespec, cena, onEdit }: {
  typNemovitosti: string; typNabidky: string; adresa: string
...
export interface NabidkaNemovitostiInitial {
  typNemovitosti?: string
  typNabidky?: 'Prodej' | 'Pronájem'
  adresa?: string
  cena?: number | string
  cenaNespec?: string
  poznamkaNemovitost?: string
  klient?: string
  makler?: string
  pobocka?: string
}
...
interface Props {
  onClose: () => void
  initial?: NabidkaNemovitostiInitial
}

const TYP_NORMALIZE: Record<string, string> = {
  dům: 'dum', byt: 'byt', pozemek: 'pozemek', komerční: 'komercni', ostatní: 'ostatni',
  dum: 'dum', komercni: 'komercni', ostatni: 'ostatni',
}

export default function NabidkaNemovitostiPanel({ onClose, initial }: Props) {
  const isEdit = !!initial
  const [showDiscard, setShowDiscard] = useState(false)
  const [klientModalOpen, setKlientModalOpen] = useState(false)

  // Step tracking — when editing, skip first step
  const [propertyConfirmed, setPropertyConfirmed] = useState(isEdit)
  const [editingProperty, setEditingProperty] = useState(false)

  // Property fields
  const [typNemovitosti, setTypNemovitosti] = useState(
    initial?.typNemovitosti ? (TYP_NORMALIZE[initial.typNemovitosti] ?? initial.typNemovitosti) : 'dum'
  )
  const [typNabidky, setTypNabidky] = useState<'Prodej' | 'Pronájem'>(initial?.typNabidky ?? 'Prodej')
  const [poznamkaNemovitost, setPoznamkaNemovitost] = useState(initial?.poznamkaNemovitost ?? '')
  const [adresa, setAdresa] = useState(initial?.adresa ?? '')
  const [cenaNespec, setCenaNespec] = useState(initial?.cenaNespec ?? 'Ne')
  const [cena, setCena] = useState(
    initial?.cena !== undefined && initial.cena !== '' && initial.cena !== 0
      ? new Intl.NumberFormat('cs-CZ').format(Number(initial.cena))
      : ''
  )

  // Client fields — split klient string if present
  const klientParts = (initial?.klient ?? '').trim().split(/\s+/)
  const [telefon, setTelefon] = useState('+420')
  const [email, setEmail] = useState('')
  const [jmeno, setJmeno] = useState(klientParts[0] ?? '')
  const [prijmeni, setPrijmeni] = useState(klientParts.slice(1).join(' '))
  ...
  const [pobocka, setPobocka] = useState(initial?.pobocka ?? POBOCKY_OPT[0]?.value ?? '')
  const [makler, setMakler] = useState(initial?.makler ?? MAKLERI_OPT[0]?.value ?? '')
        ...
        <Form
          ...
          footer={{
            actions: [
              ...
              { label: isEdit ? 'Uložit změny' : 'Vytvořit nabídku', variant: 'primary', disabled: !step2Done, onClick: onClose },
            ],
          }}
        >
          <div style={{ position: 'sticky', top: 0, zIndex: 1, padding: 24, background: 'var(--t-bgSecondary)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
            <span style={{ fontSize: 24, fontWeight: 600, lineHeight: '32px', color: 'var(--t-textPrimary)', fontFamily: 'Inter' }}>{isEdit ? 'Upravit nabídku nemovitosti' : 'Vytvořit nabídku nemovitosti'}</span>
          ...
          <div style={{ padding: '0 24px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={WIDGET}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <span style={HEADING}>Nemovitost</span>
              </div>
              ...
              {showPropertyForm && (
                <>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    <span style={HEADING}>Typ poptávky <span style={{ color: 'var(--t-textDangerPrimary)' }}>*</span></span>
                    <SwitchGroup
                      fullWidth
                      options={[
                      ...
                      value={typNabidky}
                      onChange={v => setTypNabidky(v as 'Prodej' | 'Pronájem')}
                    />
                  </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    ...
                    <TextArea
                      ...
                      value={poznamkaNemovitost}
                      onChange={setPoznamkaNemovitost}
                      width="100%"
                      minHeight={120}
                    />
                  </div>
            ...
            {step2Done && (
              <>
                ...
                <div style={WIDGET}>
                  ...
                  <TextArea placeholder="Zadejte doplňující informace k nabídce" value={poznamka} onChange={setPoznamka} width="100%" minHeight={120} />
                </div>
